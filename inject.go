...
// injectOutlines paints a silhouette-following accent ring for any part
// flagged with effects.outline. Uses the same partSilhouette() the
// connector clipper trusts, so the ring hugs the true outline of every
// shape family. Spliced just before </svg> so it sits above all faces.
func injectOutlines(svg string, infos []partInfo) string {
	tx, ty := partsScreenOrigin(infos)
	var sb strings.Builder
	any := false
	for _, p := range infos {
		if p.outlineColor == "" || p.outlineWidth <= 0 {
			continue
		}
		sil := partSilhouette(p, tx, ty)
		if len(sil) < 3 {
			continue
		}
		if !any {
			sb.WriteString(`<g data-layer="outlines" fill="none" stroke-linejoin="round">`)
			any = true
		}
		var d strings.Builder
		for i, q := range sil {
		...
		extra := ""
		if p.outlineDash != "" {
			extra += fmt.Sprintf(` stroke-dasharray="%s"`, escAttr(p.outlineDash))
		}
		if p.outlineOpacity > 0 && p.outlineOpacity < 1 {
			extra += fmt.Sprintf(` stroke-opacity="%.3f"`, p.outlineOpacity)
		}
		fmt.Fprintf(&sb, `<path d="%s" stroke="%s" stroke-width="%.2f"%s/>`,
			d.String(), escAttr(p.outlineColor), p.outlineWidth, extra)
	}
	if !any {
		return svg
	}
